    ...
    return (
        <div className="space-y-4 max-w-5xl mx-auto px-2 sm:px-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Your Appointment Ledger</h1>
            ...
            {error && (
                <div className="p-3 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 rounded-lg font-semibold text-xs">
                    ⚠️ {error}
                </div>
            )}

            {successMessage && (
                <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 rounded-lg font-bold text-xs">
                    🎉 {successMessage}
                </div>
            )}

            {appointments.length === 0 ? (
                <div className="p-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-center space-y-2">
                    <div className="text-2xl">📅</div>
                    <h3 className="font-bold text-slate-800 dark:text-slate-100 text-base">No Appointments Registered</h3>
                    <p className="text-slate-400 text-xs">Head over to the medical roster directory to schedule an appointment.</p>
                </div>
            ) : (
                <div className="bg-transparent md:bg-white md:dark:bg-slate-800 md:rounded-xl md:border md:border-slate-200 md:dark:border-slate-700 md:shadow-xs overflow-hidden">

                    <div className="hidden md:grid grid-cols-12 bg-slate-900 dark:bg-slate-950 text-slate-300 text-[11px] font-bold uppercase tracking-wider px-5 py-3 border-b border-slate-800 dark:border-slate-700">
                    ...
                    <div className="space-y-3 md:space-y-0 md:divide-y md:divide-slate-100 md:dark:divide-slate-700">
                        {appointments.map((appt) => {
                            ...
                            return (
                                <div key={appt._id} className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
                                    ...
                                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-xs md:shadow-none md:border-none md:rounded-none md:p-0 grid grid-cols-12 gap-2 md:gap-0 items-center md:px-5 md:py-3 hover:bg-slate-50/40 dark:hover:bg-slate-700/40 transition-colors text-xs text-slate-600 dark:text-slate-300">

                                        <div className="col-span-12 md:col-span-3 flex items-center space-x-3 border-b border-slate-100 dark:border-slate-700 pb-2 md:border-none md:pb-0">
                                            <img src={doctorAvatar} alt="Physician profile" className="h-8 w-8 rounded-full object-cover border border-slate-100 dark:border-slate-700 shadow-xs flex-shrink-0" />
                                            <span className="font-bold text-slate-800 dark:text-slate-100 truncate text-sm md:text-xs">Dr. {doctorName}</span>
                                        </div>
                                        ...
                                        <div className="col-span-12 md:col-span-2 flex items-center md:block mt-1 md:mt-0">
                                            ...
                                            <span className="text-[10px] font-bold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40 px-2 py-0.5 rounded-sm uppercase tracking-wide inline-block">{specialization}</span>
                                        </div>
                                        ...
                                        <div className="col-span-12 md:col-span-3 flex items-baseline md:block mt-0.5 md:mt-0 md:pr-4">
                                            ...
                                            <p className="text-slate-500 dark:text-slate-400 italic whitespace-normal break-words max-w-xs md:max-w-[220px]">
                                                "{appt.reasonForVisit || appt.problem || "Routine health evaluation."}"
                                            </p>
                                        </div>

                                        <div className="col-span-12 md:col-span-1 flex items-center justify-between md:justify-center border-t border-slate-100 dark:border-slate-700 pt-2 mt-2 md:border-none md:pt-0 md:mt-0">
                                    ...
                                    {isPanelOpen && appointmentStatus.toLowerCase() === "completed" && (
                                        <div className="bg-slate-50 dark:bg-slate-900 border-t border-b border-slate-200 dark:border-slate-700 p-4 px-6 space-y-4 animate-in slide-in-from-top-2 duration-200">

                                            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
                                                <h4 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide">Doctor's Concluding Consultation Message</h4>
                                                <p className="text-slate-400 text-[10px] mb-2">Summary remarks recorded directly by Dr. {doctorName}.</p>
                                                <p className="text-slate-700 dark:text-slate-300 font-medium text-xs bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-dashed border-slate-200 dark:border-slate-600">
                                            ...
                                            {!appt.isReviewedLocal && !appt.isReviewed ? (
                                                <form onSubmit={(e) => handleReviewSubmit(e, appt._id)} className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xs space-y-3">
                                                    <div className="space-y-0.5">
                                                        <h4 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide">Submit Practitioner Care Evaluation</h4>
                                                        <p className="text-slate-400 text-[11px]">Rate your experience to help update overall directory profile metrics.</p>
                                                    </div>

                                                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 border-t border-b border-slate-100 dark:border-slate-700 py-2.5">
                                                        <div className="flex items-center space-x-1.5">
                                                            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Score Scale:</span>
                                                            <div className="flex space-x-1">
                                                                {[1, 2, 3, 4, 5].map((num) => (
                                                                    <button
                                                                        ...
                                                                        className={`text-base transition cursor-pointer select-none focus:outline-none ${num <= rating ? "text-amber-400 scale-110" : "text-slate-200 dark:text-slate-600 hover:text-amber-200"}`}
                                                                    >
                                                                        ★
                                                                    </button>
                                                                ))}
                                                            </div>
                                                            <span className="text-xs font-black text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 px-1.5 py-0.2 rounded border border-amber-100 dark:border-amber-800 ml-1">{rating} / 5</span>
                                                        </div>
                                                    </div>

                                                    <div className="space-y-1">
                                                        <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Feedback Evaluation Narrative</label>
                                                        <textarea
                                                            value={comment}
                                                            ...
                                                            maxLength={500}
                                                            rows={2}
                                                            className="w-full border border-slate-200 dark:border-slate-600 p-2 rounded-lg text-xs text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white dark:bg-slate-700"
                                                            required
                                                        />
                                                    </div>
                                                ...
                                                <div className="bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg p-3 text-center text-slate-500 dark:text-slate-300 text-xs font-semibold italic">
